...
var Second = 1000;
var nowPlayingMusic = '';
var nowPlayingStyle = document.querySelector(".nowPlayingStyle");
var nowPlayingElement = document.querySelector(".now-playing");
document.addEventListener('DOMContentLoaded', function () {
  ...
  */
  ...
  var jqLoadAndRun = function jqLoadAndRun() {
    var $fancyboxEle = GLOBAL_CONFIG.lightbox === 'fancybox' ? document.querySelectorAll('#article-container :not(a):not(.gallery-group) > img, #article-container > img') : [];
...
function nowPlaying() {
  ...
  xhr.onload = function (response) {
    var result = JSON.parse(xhr.responseText); // update   
    ...
    nowPlayingElement.querySelector(".music-album").innerText = "".concat(result.track.artists[0].name, " - ").concat(result.track.album.name);
    SteinsColor.colors(result.track.album.picUrl, 5, function (colors) {
      ...
      nowPlayingStyle.innerHTML = "\n        .now-playing:before{\n          background-image: linear-gradient(to right, ".concat(originColor, " 50%, ").concat(opacityColor, " 80%)\n        }\n        .now-playing:after{\n          background-image: url(").concat(result.track.album.picUrl, ")\n        }\n        .now-playing .item-body .slider:before{\n          width: 50%\n        }\n        .now-playing .item-body .slider:after{\n          left: 50%\n        }\n        ");
    });
  };
}